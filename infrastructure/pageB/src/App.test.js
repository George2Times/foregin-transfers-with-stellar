...
// pageA: the two App.js files differ only in a port number and a display name,
...
function signedIn(token = "a-valid-token") {
  act(() => {
    app.setState({ token, account: "carol*bankb.com" });
  });
}
...
describe("payment()", () => {
  ...
  test.each([["-100"], ["-0.01"], ["0"], ["abc"], [""], ["  "]])(
    ...
    async (amount) => {
      signedIn();
      typeInto({ receiver: "dave*banka.com", amount });
  ...
  test("sends the receiver, the amount and the bearer token", async () => {
    signedIn("token-for-carol");
    typeInto({ receiver: "dave*banka.com", amount: "12.75" });
    global.fetch
    ...
    expect(sentBody(0)).toEqual({ receiver: "dave*banka.com", amount: "12.75" });
    expect(sentHeaders(0).Authorization).toBe("Bearer token-for-carol");
  });
  ...
  test("does not name an account in the request body", async () => {
    signedIn();
    typeInto({ receiver: "dave*banka.com", amount: "10" });
    global.fetch
      .mockReturnValueOnce(jsonResponse({ msg: "SUCCESS!", result: '{"hash":"abc123"}' }))
  ...
  test("surfaces the server's reason when it refuses the payment", async () => {
    signedIn();
    typeInto({ receiver: "dave*banka.com", amount: "10000" });
...
describe("login()", () => {
  test("requires both a friendly ID and a password", async () => {
    typeInto({ friendlyid: "carol", password: null });
  ...
  test("keeps the token and loads the account on success", async () => {
    typeInto({ friendlyid: "carol", password: "hunter2" });
    global.fetch
      .mockReturnValueOnce(jsonResponse({ msg: "SUCCESS!", token: "fresh-token" }))
      .mockReturnValueOnce(jsonResponse({ name: "Carol", balance: 100 })); // setAccount()

    await drive(() => app.login());

    expect(calledUrls()[0]).toContain("/login");
    expect(sentBody(0)).toEqual({ friendlyid: "carol", password: "hunter2" });
...
describe("setAccount()", () => {
  test("fills in the name and balance", async () => {
    signedIn();
    global.fetch.mockReturnValueOnce(jsonResponse({ name: "Carol", balance: 250.5 }));

    await drive(() => app.setAccount("carol*bankb.com"));

    expect(calledUrls()[0]).toContain("/userdet");
    expect(app.state.account).toBe("carol*bankb.com");
    expect(app.state.name).toBe("Carol");
...
describe("setBank()", () => {
  test("stores the transaction list", async () => {
    signedIn();
    const tx = [{ txid: "1001", amount: "10", sender: "dave*banka.com" }];
