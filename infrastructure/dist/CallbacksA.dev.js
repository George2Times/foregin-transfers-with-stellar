...
var conString = "postgres://bankauser:password1@localhost:5432/banka";
...
app.post("/compliance/fetch_info", function (request, response) {
  ...
  var friendlyId = addressParts[0]; // You need to create `accountDatabase.findByFriendlyId()`. It should look
  // up a customer by their Stellar account and return account information.

  client.query("SELECT name,address,dob,domain from users where friendlyid = $1", [friendlyId], function (error, results) {
    if (error) {
      throw error;
    }
...
app.post("/compliance/sanctions", function (request, response) {
  ...
  client.query("SELECT * from sanction where domain = $1", [sender.domain], function (error, results) {
    if (error) {
...
app.post("/receive", function (request, response) {
  ...
  console.log("amount", amount, "friendlyid", friendlyid); // `receive` may be called multiple times for the same payment, so check that
  // you haven't already seen this payment ID.

  var SendObj = JSON.parse(request.body.data);
  var kycObj = JSON.parse(SendObj.attachment);
  client.query("INSERT INTO transactions(txid, sender, receiver, amount, currency, kyc_info) VALUES ($1,$2,$3,$4,$5,$6)", [request.body.transaction_id, SendObj.sender, request.body.route, amount, request.body.asset_code, kycObj.transaction.sender_info], function (error, results) {
    if (error) {
      console.log(error);
    ...
    if (results) {
      ...
      client.query("SELECT balance from users where friendlyid = $1", [friendlyid], function (error, results) {
        if (error) {
          console.log(error);
        ...
        if (results) {
          ...
          client.query("UPDATE users set balance = $1 where friendlyid = $2", [balance, friendlyid], function (error, results) {
            if (error) {
              console.log(error);
              response.status(500).end("Not found");
            }
...
app.post("/test", function (request, response) {
  fetch("http://banka.com/.well-known/stellar.toml").then(function (response, error) {
    if (response) {
