// The bridge-facing callbacks, written once and configured twice.
//
// CallbacksA.js and CallbacksB.js were byte-for-byte identical apart from three
// constants at the top, so every fix -- the sanctions gate, /receive's
// idempotency and transaction, /test's missing .catch -- had to be applied twice
// by hand. Both files are now thin config that calls this factory. See
// dbserver.js for the same treatment of the bank-facing API.
//
// (Their `domain` constant is not carried over: it was declared in both files
// and read by neither.)
...
// config: { listened_port, conString }
function createCallbacks(config) {
  const app = express();
